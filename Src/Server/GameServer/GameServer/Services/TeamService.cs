...
namespace GameServer.Services
{
    class TeamService : Singleton<TeamService>
    {
        ...
        private void OnTeamInviteRequest(NetConnection<NetSession> sender, TeamInviteRequest request)
        {
            ...
            Log.InfoFormat("FowardTeamInviteRequest: : FromId:{0}:FromName{1} ToID:{2} ToName:{3}", request.FromId, request.FromName, request.ToId, request.ToName);
        ...
        private void OnTeamInviteResponse(NetConnection<NetSession> sender, TeamInviteResponse response)
        {
            ...
            if (response.Result == Result.Success)
            {
                ...
                else
                {
                    ...
                    requester.Session.Response.teamInviteRes.Errormsg = response.Request.ToName+"加入队伍";
                    requester.SendResponse();
                    sender.Session.Response.teamInviteRes.Errormsg = "成功加入队伍";
                }
            }
            sender.SendResponse();
        }
