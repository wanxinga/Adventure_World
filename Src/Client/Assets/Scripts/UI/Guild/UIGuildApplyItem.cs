...
public class UIGuildApplyItem : ListView.ListViewItem
{
    ...
    {
		MessageBox.Show(string.Format("确定要通过【{0}】的公会申请吗？", this.Info.Name), "审批申请", MessageBoxType.Confirm, "确定", "取消").OnYes = () =>
		{
	...
	{
		MessageBox.Show(string.Format("确定要拒绝【{0}】的公会申请吗？", this.Info.Name), "审批申请", MessageBoxType.Confirm, "确定", "取消").OnYes = () =>
		{
